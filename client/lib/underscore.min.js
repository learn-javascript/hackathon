...
var r=n.constructor,e=ju.call(n),u=e==dt,o=e==At;if(!o||typeof r=="function"&&r instanceof r||(r=gu),u||o){var i=t?new r:Xt(new r,n);return u&&(i.length=n.length),i}switch(e){case kt:return kr(n);case mt:case _t:return new r(+n);case Et:case Ot:case Rt:case Ct:case St:case Tt:case Nt:case Wt:case Ft:return e=n.buffer,new r(t?kr(e):e,n.byteOffset,n.length);case xt:case It:return new r(n);case jt:i=new r(n.source,ut.exec(n)),i.lastIndex=n.lastIndex}return i}function qr(n){return w(n)&&Kr(n.length)&&Ut[ju.call(n)]||false
...
var fu=x.Array,lu=x.Date,cu=x.Error,su=x.Function,pu=x.Math,hu=x.Number,gu=x.Object,vu=x.RegExp,du=x.String,yu=x.TypeError,mu=fu.prototype,_u=gu.prototype,bu=(bu=x.window)&&bu.document,wu=su.prototype.toString,xu=_u.hasOwnProperty,Au=x._,ju=_u.toString,Iu=vu("^"+Ge(ju).replace(/toString|(function).*?(?=\\\()| for .+?(?=\\\])/g,"$1.*?")+"$"),ku=De(ku=x.ArrayBuffer)&&ku,Eu=De(Eu=ku&&new ku(0).slice)&&Eu,Ou=pu.ceil,Ru=x.clearTimeout,Cu=pu.floor,Su=De(Su=gu.getPrototypeOf)&&Su,Tu=mu.push,Nu=_u.propertyIsEnumerable,Wu=De(Wu=x.Set)&&Wu,Fu=x.setTimeout,Uu=mu.splice,$u=De($u=x.Uint8Array)&&$u,Bu=De(Bu=x.d)&&Bu,Lu=function(){try{var n=De(n=x.Float64Array)&&n,t=new n(new ku(10),0,1)&&n
